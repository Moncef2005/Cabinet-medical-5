...
<div class="card mb-3 border-0" style="background:linear-gradient(135deg,#198754,#4caf50);color:#fff">
    <div class="card-body p-4 d-flex align-items-center gap-4">
        ...
        <div>
            <h5 class="mb-0 fw-bold">{{ $appointment->patient->user->name }}</h5>
            <div class="opacity-75" style="font-size:.875rem">
                RDV du {{ $appointment->scheduled_at->format('d/m/Y à H:i') }} · Motif : {{ $appointment->reason }}
                @if($appointment->patient->age)
                · {{ $appointment->patient->age }} ans · {{ $appointment->patient->blood_type }}
                @endif
            </div>
        </div>
    </div>
...
<form action="{{ route('consultations.store', $appointment) }}" method="POST" id="consultationForm">
    @csrf
            ...
            <div class="card mb-3">
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" name="chief_complaint" class="form-control @error('chief_complaint') is-invalid @enderror"
                               value="{{ old('chief_complaint', $appointment->reason) }}" required>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="diagnosis" class="form-control @error('diagnosis') is-invalid @enderror" rows="3"
                                  placeholder="Diagnostic médical..." required>{{ old('diagnosis') }}</textarea>
                        @error('diagnosis')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div>
                        <label class="form-label fw-semibold">Notes & observations</label>
                        <textarea name="notes" class="form-control" rows="3"
                                  placeholder="Observations cliniques, recommandations...">{{ old('notes') }}</textarea>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    <div class="row g-3">
                        <div class="col-6">
                            ...
                            <input type="number" name="weight" class="form-control form-control-sm" placeholder="70" min="1" max="500" step="0.1" value="{{ old('weight') }}">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="font-size:.82rem">Taille (cm)</label>
                            <input type="number" name="height" class="form-control form-control-sm" placeholder="170" min="30" max="250" value="{{ old('height') }}">
                        </div>
                        <div class="col-12">
                            <label class="form-label fw-semibold" style="font-size:.82rem">Tension artérielle</label>
                            <div class="input-group input-group-sm">
                                <input type="number" name="blood_pressure_systolic" class="form-control" placeholder="120" min="50" max="300" value="{{ old('blood_pressure_systolic') }}">
                                <span class="input-group-text">/</span>
                                <input type="number" name="blood_pressure_diastolic" class="form-control" placeholder="80" min="20" max="200" value="{{ old('blood_pressure_diastolic') }}">
                                <span class="input-group-text">mmHg</span>
                            </div>
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="font-size:.82rem">Température (°C)</label>
                            <input type="number" name="temperature" class="form-control form-control-sm" placeholder="37.0" min="34" max="42" step="0.1" value="{{ old('temperature') }}">
                        </div>
                        <div class="col-6">
                            <label class="form-label fw-semibold" style="font-size:.82rem">Pouls (bpm)</label>
                            <input type="number" name="heart_rate" class="form-control form-control-sm" placeholder="72" min="20" max="300" value="{{ old('heart_rate') }}">
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="card-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="number" name="price" class="form-control form-control-sm"
                               value="{{ old('price', $appointment->doctor->consultation_price) }}" min="0" step="10">
                    </div>
                    <div>
                        ...
                        <select name="payment_status" class="form-select form-select-sm" required>
                            ...
                            <option value="insurance" {{ old('payment_status')==='insurance' ?'selected':'' }}>Assurance</option>
                        </select>
                    </div>
                </div>
            </div>
