...
<head>
    <meta charset="UTF-8">
    <title>Compte-rendu consultation #{{ $consultation->id }}</title>
    <style>
...
<div class="page">
    <div class="header">
        ...
        <div class="doc-info">
            <h3>Dr. {{ $consultation->doctor->user->name }}</h3>
            <p class="specialty">{{ $consultation->doctor->specialty }}</p>
            <p>N° Ordre: {{ $consultation->doctor->license_number }}</p>
        </div>
    </div>
    ...
    <div class="meta-grid">
        <div class="meta-box">
            ...
            <div class="value">{{ $consultation->patient->user->name }}</div>
        </div>
        <div class="meta-box">
            <div class="label">Date</div>
            <div class="value">{{ $consultation->created_at->format('d/m/Y') }}</div>
        </div>
        <div class="meta-box">
            <div class="label">Réf. consultation</div>
            <div class="value">#{{ $consultation->id }}</div>
        </div>
    </div>
    ...
    <div class="section">
        <h4>Diagnostic</h4>
        <div class="diagnosis-box"><p>{{ $consultation->diagnosis }}</p></div>
    </div>
    ...
    @if($consultation->weight || $consultation->temperature || $consultation->blood_pressure_systolic || $consultation->heart_rate)
    <div class="section">
        <h4>Signes vitaux</h4>
        <div class="vitals-grid">
            @if($consultation->weight)<div class="vital"><div class="v-label">Poids</div><div class="v-value">{{ $consultation->weight }} kg</div></div>@endif
            @if($consultation->height)<div class="vital"><div class="v-label">Taille</div><div class="v-value">{{ $consultation->height }} cm</div></div>@endif
            @if($consultation->bmi)<div class="vital"><div class="v-label">IMC</div><div class="v-value">{{ $consultation->bmi }}</div></div>@endif
            @if($consultation->temperature)<div class="vital"><div class="v-label">Temp.</div><div class="v-value">{{ $consultation->temperature }}°C</div></div>@endif
            @if($consultation->heart_rate)<div class="vital"><div class="v-label">Pouls</div><div class="v-value">{{ $consultation->heart_rate }} bpm</div></div>@endif
            @if($consultation->blood_pressure_systolic)<div class="vital"><div class="v-label">Tension</div><div class="v-value">{{ $consultation->blood_pressure }}</div></div>@endif
        </div>
    </div>
    @endif

    @if($consultation->prescription && $consultation->prescription->items->count() > 0)
    <div class="section">
    ...
    <div class="footer">
        <div style="font-size:9pt;color:#4a5568">
            Casablanca, le {{ $consultation->created_at->format('d/m/Y') }}
        </div>
        <div class="signature">
